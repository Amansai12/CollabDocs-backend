...
docRouter.get('/version/:docId',verifyToken, async (req,res) => {
    ...
    try{
        ...
        res.status(200).json({message:"Versions fetched successfully",versions})
    }catch(error){
        console.log(error)
        res.status(500).json({message:"Something went wrong"})
    }
...
docRouter.put('/save',verifyToken, async (req,res) => {
    ...
    try{
        ...
        res.status(200).json({message:"Document updated successfully",updatedDoc})
    }catch(error){
        console.log(error)
        res.status(500).json({message:"Something went wrong"})
    }
...
docRouter.post('/rollback/:docId',verifyToken, async (req,res) => {
    ...
    try{
        ...
        res.status(200).json({message:"Version set successfully",updatedDoc,version})
    }catch(error){
        console.log(error)
        res.status(500).json({message:"Something went wrong"})
    }
...
docRouter.post('/generate-link',verifyToken, async (req,res) => {
    ...
    try{
        ...
        if(doc.ownerId !== ownerId){
            return res.status(401).json({message:"You are not the owner of this document"})
        }
